...
export async function getDashboardReport(): Promise<DashboardReport> {
  ...
  // Calculate active and completed contracts
  ...
  const avgPhysicalProgress = contractsWithPhysicalProgress.length > 0 
    ? contractsWithPhysicalProgress.reduce((sum, contract) => {
        const lastProgress = contract.physicalProgress.sort((a, b) => 
  ...
  const avgFinancialProgress = contractsWithFinancialProgress.length > 0 
    ? contractsWithFinancialProgress.reduce((sum, contract) => 
  ...
  contracts.forEach(contract => {
    if (contract.sumberDana) {
      ...
      const currentValue = fundingSourceMap.get(source) || { count: 0, amount: 0 }
  ...
  const fundingSourceDistribution = Array.from(fundingSourceMap.entries()).map(([source, data]) => ({
    source,
    count: data.count,
    amount: data.amount
  }))
  ...
  const problemContracts = contracts
    ...
    .map(contract => {
      const latestPhysicalProgress = contract.physicalProgress.sort((a, b) => 
        new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
      )[0]
      ...
      return {
        ...
        issueDetails: {
          ...
          progressImpact: latestPhysicalProgress ? `${latestPhysicalProgress.deviasi.toFixed(2)}%` : undefined,
...
// Helper functions
function getContractStatus(contract: Contract & {physicalProgress: PhysicalProgress[]}): string {
  const today = new Date()
  
  if (!contract.startDate) return 'BELUM DIMULAI'
  if (contract.endDate && contract.endDate < today) {
    const latestProgress = contract.physicalProgress.sort((a, b) => 
      new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
    )[0]
...
function determineIssueType(contract: any): 'KENDALA' | 'KETERLAMBATAN' | 'PERPANJANGAN' | 'LAINNYA' {
  if (contract.permasalahan && contract.permasalahan.toLowerCase().includes('terlambat')) {
...
function determineIssueSeverity(contract: any, latestProgress: any): 'RINGAN' | 'SEDANG' | 'BERAT' {
