...
const ContractDurationCard: React.FC<ContractDurationCardProps> = ({
  form,
}) => {
  const calculateTotalDays = (): number => {
    const baseDays: number = form.watch("masaPelaksanaan") || 0;

    const regularAddendumDays: number =
      form
        .watch("addendum")
        ?.filter(
          (item: Addendum) =>
        ...
        .reduce(
          (acc: number, item: Addendum) => acc + Number(item.hari || 0),
          0
        ) || 0;

    const specialAddendumDays: number =
      form
        .watch("addendum")
        ?.filter(
          (item: Addendum) =>
  ...
  const regularAddendums: Addendum[] =
    form
      .watch("addendum")
      ?.filter((item: Addendum) => !item.pemberianKesempatan) || [];

  const specialAddendums: Addendum[] =
    form
      .watch("addendum")
      ?.filter((item: Addendum) => item.pemberianKesempatan) || [];
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-12 gap-2 items-center text-sm">
            <div className="col-span-4 font-medium">Akhir Kontrak Asli</div>
              ...
              <div className="col-span-3 text-right">
                <span className="px-2 py-1 bg-gray-100 rounded-md">
                  {regularAddendums
                    ...
                    .filter(
                      (item: Addendum) =>
                        Number(item.hari) && item.tipe === "waktu"
                    )
                    .reduce(
                      (acc: number, item: Addendum) =>
