...
export default function AddendumStep() {
  ...
  // Effect untuk memperbarui form saat local state berubah
  useEffect(() => {
  ...
  // Effect untuk menangani perubahan pada checkbox hasAddendum
  useEffect(() => {
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center space-x-2 space-y-0">
                    ...
                    <FormLabel className="font-medium cursor-pointer m-0">Proyek Memiliki Addendum</FormLabel>
                  </FormItem>
                )}
              />
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    <p>Addendum adalah perubahan atau tambahan terhadap kontrak asli yang disepakati oleh para pihak setelah kontrak ditandatangani.</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
        ...
        <CardContent className="pt-4">
            ...
            <div className="space-y-6">
              ...
              {addendumItems.length === 0 ? (
                <div className="text-center py-8 border border-dashed rounded-md bg-slate-50">
                  <p className="text-slate-500">Belum ada addendum yang ditambahkan</p>
                  <Button 
                    type="button" 
                    variant="outline" 
                    size="sm" 
                ...
                <div className="space-y-6">
                  {addendumItems.map((item, index) => (
                    <Card key={item.id} className={cn(
                      ...
                      !item.name && !item.tipe ? "border-orange-200 bg-orange-50" : "border-slate-200"
                    )}>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {item.tipe && (
                            <Badge variant="outline" className={cn(
                              "text-xs py-0 h-5",
                              item.tipe === "waktu" ? "border-blue-500 text-blue-600 bg-blue-50" :
                              item.tipe === "volume" ? "border-green-500 text-green-600 bg-green-50" :
                              "border-slate-500 text-slate-600 bg-slate-50"
                            )}>
                              {ADDENDUM_TYPES.find(t => t.value === item.tipe)?.label || item.tipe}
                            </Badge>
                          )}
                        </div>
                        <Button
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <Input
                                ...
                                value={item.name || ""}
                                onChange={(e) => updateAddendumItem(index, "name", e.target.value)}
                                className={!item.name ? "border-orange-300" : ""}
                              />
                            </FormControl>
                          </FormItem>
                          
                          <FormItem>
                            <FormLabel className="text-sm font-medium">Tipe Addendum</FormLabel>
                            <Select
                              value={item.tipe || ""}
                              onValueChange={(value) => updateAddendumItem(index, "tipe", value)}
                            >
                              <SelectTrigger className={!item.tipe ? "border-orange-300" : ""}>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          {/* <FormItem>
                            ...
                            <DatePicker
                              ...
                              onSelect={(date) => updateAddendumItem(index, "tanggalAddendum", date)}
                              className="w-full"
                              ...
                              <FormItem>
                                ...
                                <FormControl>
                                  <Input
                                    ...
                                    value={item.hari || ""}
                                    onChange={(e) => updateAddendumItem(index, "hari", e.target.value)}
                                  />
                                </FormControl>
                            ...
                            <FormItem>
                              ...
                              <FormControl>
                                <Input
                                  ...
                                  value={item.volume || ""}
                                  onChange={(e) => updateAddendumItem(index, "volume", e.target.value)}
                                />
                              </FormControl>
                            </FormItem>
                            
                            <FormItem>
                              <FormLabel className="text-sm font-medium">Satuan</FormLabel>
                              <FormControl>
                                <Input
                                  placeholder="Contoh: m³, ton, unit"
                                  value={item.satuan || ""}
                                  onChange={(e) => updateAddendumItem(index, "satuan", e.target.value)}
                                />
                              </FormControl>
                            </FormItem>
                          </div>
          ...
          {!form.watch("hasAddendum") && (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <h3 className="text-lg font-medium text-slate-700">Tidak Ada Addendum</h3>
              <p className="text-slate-500 max-w-md mt-1">
                Aktifkan opsi &quot;Proyek Memiliki Addendum&quot; jika terdapat perubahan kontrak setelah penandatanganan
              </p>
            </div>
          )}
        </CardContent>
